...
sc::GridGraph::GridGraph(cv::Point3d origin, cv::Point3d resolution, cv::Point3d state_min, cv::Point3d state_max, const cv::Mat &map)
{
    this->origin = origin;
...
void sc::GridGraph::initDistanceToObstacle()
{
...
void sc::GridGraph::insertObstacle(const cv::Point2i &index)
{
...
void sc::GridGraph::removeObstacle(const cv::Point2i &index)
{
    if(index.x < 0 || index.x >= size.x || index.y < 0 || index.y >= size.y)
    {
        return;
...
void sc::GridGraph::updateObstacle(const std::vector<cv::Point2i> &map)
{
    for(auto & p : map)
    {
...
void sc::GridGraph::updateDistanceToObstacle()
{
    while(!open_set.empty())
    {
        GridNode* current = open_set.begin()->second;
...
PlanNode *sc::GridGraph::operator()(int x, int y, int z)
{
...
PlanNode *sc::GridGraph::operator()(const cv::Point3i &index)
{
    return this->operator()(index.x, index.y, index.z);
}

PlanNode *sc::GridGraph::operator()(double x, double y, double z)
{
...
PlanNode *sc::GridGraph::operator()(const cv::Point3d &state)
{
    return this->operator()(state.x, state.y, state.z);
}

GridNode *sc::GridGraph::operator()(int x, int y)
{
...
GridNode *sc::GridGraph::operator()(const cv::Point2i &index)
{
    return this->operator()(index.x, index.y);
}

GridNode *sc::GridGraph::operator()(double x, double y)
{
    int ix = (x - state_min.x) / resolution.x;
    int iy = (y - state_min.y) / resolution.y;
...
GridNode *sc::GridGraph::operator()(const cv::Point2d &state)
{
    return this->operator()(state.x, state.y);
}

bool sc::PlanNodeCost::operator<(const PlanNodeCost &other) const
{
